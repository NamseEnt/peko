...
const { InterruptedPrototype, isArrayBuffer } = core;
  ...
  start() {
    ...
    (async () => {
      ...
      while (true) {
        ...
        let data;
        try {
          this[_dataPromise] = op_message_port_recv_message(this[_id]);
          if (
...
function serializeJsMessageData(data, transferables) {
  ...
  if (transferables.length > 0) {
    ...
    for (let i = 0, j = 0; i < transferables.length; i++) {
      ...
      if (isArrayBuffer(t)) {
        if (ArrayBufferPrototypeGetByteLength(t) === 0 && isDetachedBuffer(t)) {
          throw new DOMException(
